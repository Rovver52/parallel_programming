...
def read_matrix_mpi(filename):
    """Чтение результата из файла MPI программы"""
    try:
...
def main():
    if len(sys.argv) != 4:
        print("Usage: python verify_mpi.py <matrix_A> <matrix_B> <result_file>")
        sys.exit(1)

    file_a, file_b, file_res = sys.argv[1], sys.argv[2], sys.argv[3]

    print("=== Verification (MPI result) ===")

    A = read_input_matrix(file_a)
    B = read_input_matrix(file_b)
    C_mpi, n = read_matrix_mpi(file_res)

    if A is None or B is None or C_mpi is None:
    ...
    max_diff = np.max(np.abs(C_mpi - C_numpy))
    if np.allclose(C_mpi, C_numpy, rtol=1e-5, atol=1e-8):
